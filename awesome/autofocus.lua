...
local function check_focus(obj)
    ...
    if not client.focus or not client.focus:isvisible() then
        ...
        -- while ((not c) or c.sticky) and (idx < 100) do
        --     idx = idx + 1
        --     c = aclient.focus.history.get(s, idx)
        -- end
        if c and not c.sticky then 
            -- naughty.notify({text = tostring(c.sticky)})
            -- naughty.notify({text = tostring(idx)})
            client.focus = aclient.focus.filter(c)
        end
    end
end
...
local function check_focus_tag(t)
    ...
    if client.focus and client.focus.screen ~= s then
        local idx = 0
        local c = aclient.focus.history.get(s, idx)
        while ((not c) or c.sticky) and (idx < 10) do
            idx = idx + 1
            c = aclient.focus.history.get(s, idx)
        end
        if c and not c.sticky  then 
