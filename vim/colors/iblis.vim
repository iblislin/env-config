...
" Both palettes are applied unconditionally, and that is deliberate.  They used
" to sit in `if has("gui_running") / else`, so a terminal only ever got the cterm
" half -- and any editor running with 'termguicolors' on then fell back to its own
" defaults.  That is why python `from ... import ...` rendered grey in nvim (which
" negotiates truecolor on) and yellow in vim (which does not).
"
" A :highlight command only touches the colour space it names, so the two halves
" compose.  ORDER MATTERS: cterm comes first because it does `hi clear` on the
" Spell* groups, which would wipe the gui undercurl if it ran second.
...
" These four are cterm-only in origin; the gui values are the xterm palette
" entries for the very indices above (81 -> #5fd7ff, cyan=14 -> #00ffff,
" red=9 -> #ff0000, 4 -> #0000ee), measured with t_Co=256, so terminal and
" truecolor render the same hue rather than a new choice being invented here.
